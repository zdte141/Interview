...
public class Fix23 {
  ...
  public List<Integer> fix23(int[] nums) {
    // create the arrayList test
    List<Integer> test = new ArrayList<>();
    // add the values into the arrayList.
    for (int temp : nums) {
      test.add(temp);
    }
    // dynamic code to which can implement all length of arrays.
    for (int i = 0; i < test.size() - 1; i++) {
      // if the index number contains 2 and following one is 3 then set the 3 to 0.
      if (test.get(i) == 2 && test.get(i + 1) == 3) {
